<template>
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-white">Tickets - Criação de Scripts</h1>
        ...
        <div v-if="showCreateDialog" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 px-4" style="backdrop-filter: blur(4px);">
            <div class="bg-neutral-800 rounded-lg shadow-lg w-full max-w-3xl mx-auto max-h-[90vh] overflow-y-auto">
                <div class="p-6 border-b border-neutral-700 flex justify-between items-center sticky top-0 bg-neutral-800 z-10">
                    <h3 class="text-lg font-medium text-white">Novo Ticket - Criação de Scripts</h3>
                ...
                <form @submit.prevent="createTicket" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            required
                            rows="6"
                            placeholder="Descreva detalhadamente o script que precisa ser criado..."
...
const createTicket = async () => {
    if (!form.value.campaignId || !form.value.description) {
        showNotification('error', 'Por favor, preencha todos os campos obrigatórios (Campanha e Descrição).');
        return;
    }
...
const loadTickets = async () => {
    loading.value = true;
    try {
        // Buscar apenas tickets do tipo script-creation (filtro no backend)
        const response = await client.tickets.get({ ticketType: 'script-creation' });
