<template>
    <div class="flex flex-col min-h-screen bg-gray-100">
        <!-- Header -->
        <header class="bg-[#4b0d00] sticky top-0 z-50 shadow-md">
            <div class="max-w-[1200px] mx-auto px-4">
                <div class="flex justify-between items-center h-14">
                    <!-- Logo -->
                    <div class="flex-shrink-0">
                        <a href="/" class="text-2xl font-bold text-white">
                            <img src="/logo-pitada.png" width="147" height="32" alt="Logo" title="Logo">
                        </a>
                    </div>
                        ...
                        <template v-for="category in mainNavCategories.rootCategories" :key="category.id">
                            <div v-if="mainNavCategories.childrenMap[category.id]" class="relative">
                                <button
                                    ...
                                    class="dropdown-toggle text-white hover:bg-[#064019] dark:hover:bg-[#4b0d00] hover:text-[#ffcc00] px-2 py-1 rounded text-sm flex items-center whitespace-nowrap transition-colors"
                                ...
                                <div
                                    ...
                                    class="dropdown-menu absolute left-0 mt-1 w-48 rounded-md shadow-lg bg-[#4b0d00] ring-1 ring-black ring-opacity-5 z-10"
                                >
                            ...
                            <a v-else
                                ...
                                class="text-white hover:bg-[#064019] dark:hover:bg-[#4b0d00] hover:text-[#ffcc00] px-2 py-1 rounded text-sm whitespace-nowrap transition-colors"
                            >
                                {{ category.name }}
                            </a>
                        </template>
                    ...
                    <div class="flex flex-col gap-1">
                        <template v-for="category in mainNavCategories.rootCategories" :key="category.id">
                            <div v-if="mainNavCategories.childrenMap[category.id]" class="w-full">
                                <button
                                    @click="(e) => toggleDropdown(category.id, e)"
                                    class="dropdown-toggle flex items-center justify-between w-full text-white hover:bg-[#064019] dark:hover:bg-[#4b0d00] hover:text-[#ffcc00] rounded px-3 py-2 text-sm"
                                    :class="{'bg-[#064019] text-[#ffcc00]': openDropdowns[category.id]}"
                                >
                                    {{ category.name }}
                                    <svg class="ml-1 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div v-show="openDropdowns[category.id]" class="pl-4 py-1 bg-[#4b0d00] rounded mt-1">
                                    <a
                                        v-for="child in mainNavCategories.childrenMap[category.id]"
                                        :key="child.id"
                                        :href="`/category/${child.slug}`"
                                        class="block px-4 py-2 text-sm text-white hover:bg-[#064019] hover:text-[#ffcc00] rounded"
                                    >
                                        {{ child.name }}
                                    </a>
                                </div>
                            ...
                            <a
                                v-else
                                :href="`/category/${category.slug}`"
                                class="block text-white hover:bg-[#064019] dark:hover:bg-[#4b0d00] hover:text-[#ffcc00] rounded px-3 py-2 text-sm"
                            >
                                {{ category.name }}
                            </a>
                        </template>
        ...
        <!-- Newsletter Section -->
        <section class="bg-[#4b0d00] py-10 text-white mt-8">
            <div class="max-w-[1200px] mx-auto px-4">
            ...
            <div class="max-w-[1200px] mx-auto px-4 py-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <h3 class="text-lg font-bold mb-4 relative pb-3">
                            Sobre o Pitada Gourmet
                            <span class="absolute bottom-0 left-0 w-10 h-1 bg-[#4b0d00]"></span>
                        </h3>
                ...
                <div class="border-t border-gray-700 mt-8 pt-6 text-center">
                    <p class="text-gray-400 text-sm">
                        &copy; {{ new Date().getFullYear() }} Pitada Gourmet - Todos os direitos reservados.
                    </p>
                </div>
            </div>
        </footer>
                            ...
                            <div class="w-full">
                                ...
                                <div class="mb-4 relative">
                                    ...
                                    <input
                                        ...
                                        class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg block w-full pl-10 p-2.5 focus:ring-[#4b0d00] focus:border-[#4b0d00]"
                                ...
                                <div class="mt-4 max-h-[70vh] overflow-y-auto">
                                    <div v-if="isSearching" class="flex justify-center items-center py-8">
                                        <div class="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#4b0d00]"></div>
                                    </div>
...
useHead({
    ...
    link: [
        {
            rel: 'stylesheet',
            href: '/src/theme-pitadagourmet/style.css'
        },
        {
            rel: 'icon',
            type: 'image/ico',
            href: '/src/theme-pitadagourmet/assets/favicon.ico'
        },
