<template>
    <div class="w-full max-w-[1200px] mx-auto px-4 py-8">
        ...
        <div v-else-if="!category" class="text-center py-10 bg-white rounded-lg p-6">
            ...
            <a href="/cupom" class="mt-4 inline-block bg-[#adadb0] hover:bg-indigo-700 text-white px-4 py-2 rounded-md transition-colors">
...
const headData = computed(() => ({
    ...
    meta: [
        ...
        {
            property: 'og:url',
            content: `${settings.value?.['blog.url'] || 'https://oferta.guru'}/categoria/${route.params.slug}`
        }
    ]
}));
